...
| `get_in_memory_memory`   | 🗂️ 创建会话内存实例 | [CoPawInMemoryMemory](reme/memory/file_based/copaw_in_memory_memory.py) — Token 感知的内存管理，支持压缩摘要和状态序列化     |
...
`ReMeCopaw` 环境变量配置 Embedding 和存储后端
...
[CoPawInMemoryMemory](reme/memory/file_based/copaw_in_memory_memory.py) 扩展了 AgentScope 的 `InMemoryMemory`：
